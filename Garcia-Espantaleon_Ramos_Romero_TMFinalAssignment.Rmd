...
author: " Irene García-Espantaleón Artal, Jorge Ramos Val and Pablo Romero Medinilla"
date: "2025-04-25"
output:
...
extract patterns and insights from large volumes of textual data (in
this case, Kanye's lyrics). To achieve this, three different techniques
have been selected:
...
2.  Insult Analysis: The objective is to identify, quantify and
    characterize the use of offensive language, such as profanities,
    slurs and insults, across Kanye’s discography. Using a curated
    lexicon of “bad words”, we compute the relative frequency of insults
    per album, track their evolution and examine how spikes in coarse
    language correlate with overall sentiment and thematic intensity.

3.  Word co-occurences. The objective is to build and analyze networks
    of co‑occurring terms within each line across the lyrics. By
    treating each word as a node and each co‑occurrence as a weighted
    edge, we uncover thematic clusters and explore what kind of language
    is used in each of them.

In addition, tools like Term Frequency and TF-IDF will be used in the
introductory part of this exercise to get a general insight of our
collection and conduct an exploratory comparison of all the albums
comprised.
...
of 98 Kanye West songs, covering his discography from 2004 to 2018. We
consider this a long enough period to account for a good part of Kanye's
musical evolution and the personal shifts behind it. However, we'd like
to acknowledge that most of the artist's more controversial turns have
happened in the last few years.

It is worth noting that the original CSV file does not include album
identification or release years, so this information will need to be
added manually to conduct the analysis.
...
With these additions, we now have the necessary information to carry out
the analysis. We can check for missing values to see if all of our rows
were filled correctly through this method.
...
As observed, there are a total of 9 albums and one song that does not
belong to any, meaning it is a single. We don't know why it was included
in the dataset, because it is not the only single released by the artist
in this time period. Since we're interesed in working at the album
level, we will drop this observation now:
...
1.  Punctuation will be removed, since it does not add value to a text
    analysis like this.
...
meaning frequently used in music.
...
```{r}
# Visualization: top 10 words per album by TF-IDF
...
First, it will be useful to explore a little the chosen lexicon, to
account for any bias or imbalances that might be reflected on our
analysis.

```{r}
head(nrc_lexicon, n = 10)
```

The first important thing we notice is that this is a multilabel
classification, where each word might be assigned to more than one
emotion. Furthermore, we can see that "negative" is included as a label,
which might indicate that this lexicon mixes sentiments and emotions.
This is beneficial in that it captures the ambiguity and complexity of
language, where most words are nuanced and can take on different
meanings and reflect different emotions depending on the context. On the
other hand, this could bias our results, since if each word contributes
to several emotions, our totals may inflate the apparent emotional
content.

We might also check if this lexicon is systematically biased towards a
certain sentiment or emotion:
...
As expected, negative and positive categories are more frequent labels,
probably because sentiments are broader, less specific than emotions.
From what we saw on the previous table, we can also anticipate that
labels like "fear", "anger" and "sadness" are likely to be used
alongside the "negative" sentiment. Those are also more prominent in the
lexicon than positive emotions such as "joy" or "trust", which probably
indicates that the lexicon is skewed towards negative categories. This
could lead our analysis to exaggerate negativity in Kanye's lyrics, as
well as undervalue positive or neutral words.

Bearing this in mind will allow us to make a better, more nuanced
interpretation of our results.

```{r}
# Count the frequency of each sentiment in the lyrics (NRC lexicon)
kanye_nrc %>%
  distinct(word, sentiment) |> # Keep only unique word-sentiment pairs
  count(sentiment, sort = TRUE) %>%
  ...
  geom_text(aes(label = n), hjust = -0.1, size = 3) + # Add count labels
   scale_fill_gradient(low = "#cce5ff", high = "#003366") +  # Apply gradient color palette
...
As we had anticipated, positive and negative sentiments dominate Kanye's
lyrics, although it is interesting to observe a preeminence of the
positive category, seeing that negative words outnumbered positive ones
on the original lexicon by around 1000. This becomes more meaningful
when we note that trust, a rather positive emotion, is the most frequent
in our analysis, though followed closely by a rather negative one, fear.
We can also notice a significant balance between anger, joy, sadness and
anticipation. This plot considers unique word-sentiment pairs, so only
distinct words contribute to sentiments, and not their frequency. We can
easily remove one line from our code so that it reflects repeated uses
of words:
...
In this case, positive words also outnumber negative ones, but the gap
between these sentiment categories and the rest of emotions is widened,
suggesting that Kanye commonly uses words that have some sentiment but
no emotion assigned (maybe generic language). Trust remains the most
frequent emotion, and joy's rank goes up. This suggests that Kanye
doesn't employ a very varied vocabulary to express joy, but rather
reuses joyful words in his lyrics. The same thing happens to words
expressing anger. On the other hand, fear and sadness as emotions
notably drop in the ranking, indicating that Kanye has a richer
vocabulary to convey these emotions, but he doesn't use it as
frequently.
...
kanye_nrc %>%
  ...
  # Create a rank variable for ordering
  mutate(sentiment_rank = rank(-n)) %>%
  ungroup() %>%
  mutate(
    album = factor(album, levels = album_order),  # Ensure chronological album order in facets
    # Create a new variable for plotting that combines sentiment with album
    sentiment_plot = paste(sentiment, album, sep = "___")
  ) %>%
  # Reorder the plotting variable by rank within each album
  mutate(sentiment_plot = reorder(sentiment_plot, -sentiment_rank)) %>%
  ggplot(aes(x = n, y = sentiment_plot, fill = sentiment)) +  # Use sentiment_plot for y, original sentiment for fill
  geom_col(show.legend = FALSE) +  # Bar chart without legend
  facet_wrap(~ album, scales = "free", ncol = 5) +  # Facet by album with independent y-axis
  # Clean up the y-axis labels to show original sentiment names
  scale_y_discrete(labels = function(x) gsub("(.*)___.*", "\\1", x)) +
  scale_fill_manual(values = sentiment_colors) +  # Use your custom sentiment colors
  labs(
...
observed. Throughout his career, we observe a predominance of positive
and negative sentiments, which is consistent with our previous
conclusions. The positive category dominates in the three albums from
2004 to 2007, and then there's a switch to negative for another three
albums (2008-2013), although the frequencies are more balanced.

Additionally, sentiments such as joy and trust regularly stand out in
several albums, while anger, fear, disgust and sadness are included in
two of the 9 ranks, and anticipation appears in only one. The positive
and negative emotions as we have described them do not seem to show a
pattern, meaning that, for example, joy can co-occur in the ranking
alongside trust, another positive emotion, but also fear or sadness. In
sum, all of Kanye's albums have a somewhat balanced mix of positive and
negative emotions.

### b. BING

This section conducts a binary classification between words considered
positive and negative.

First, let's have a first look at the lexicon:

```{r}
head(bing_lexicon, n = 15)
```

We should also check if this classification is multilabel as the
previous one:

```{r}
bing_lexicon %>%
  group_by(word) %>%
  summarize(n_labels = n_distinct(sentiment), labels = paste(unique(sentiment), collapse = ", ")) %>%
  filter(n_labels > 1)
```

There are only three words, all from the same family, that have the two
possible labels attached. Although this is questionable, it doesn't seem
like it will influence our analysis.

Lastly, let's check for bias in our lexicon:
...
The BING lexicon shows a clear skew towards negative sentiment, with
more than twice as many terms as for the positive class. There is no
"neutral" category.

Now we can move on to visualizing the sentiment of Kanye's lyrics:
...
predominance of words considered negative. This result appears to
contradict the previous section's findings, which might be attributed to
the negative skew" that we proved.

Now, let's examine what this classification looks like by album:
...
Negative words are more frequent than positive ones for all of our
albums except one: Kids See Ghosts, for which we only have two songs.
This is consistent with the first graph of this section but, as that one
did, contradicts the conclusions drawn from analysis using the NRC
lexicon. It is worth noting that he gap appears to be larger in more
recent albums (with the exception of the aforementioned one), suggesting
a trend towards imbalance in favour of negative sentiment.

### c. AFINN

In this part, we will analyze the evolution of sentiment using the AFINN
lexicon scores (which quantifies the sentiment of words from -5 to 5).

```{r}
head(afinn_lexicon, n = 15)
```

We can check how many words are assigned to each value:
...
We observe that the AFINN lexicon is skewed toward moderately negative
values, particularly -2, which appears 966 times. Extreme negative
scores (like -4 or -5 and 4 or 5) are relatively rare. Only one word was
assigned a value of 0 (neutrality) in this lexicon.
...
negative score, which seems to confirm what we saw earlier. My Beautiful
Dark Twisted Fantasy has the lowest score by far. This album started the
trend described in the previous section of a widening gap between
positive and negative words. It also showed a preeminence of negative
sentiment according to the NRC lexicon.

On the other hand, we see that there is a positive total score in only
two cases: - 808 & Heartbreak (2008), which has the highest score. This
album had a higher frequency of negative words according to BING, but
the difference was the smallest. Positive sentiments also dominated
according to NRC. - Kids See Ghosts (2018). This album also revealed a
predominance of positive words in the two previous execises.

What's been done earlier is in absolute terms, which can distort the
reality since some albums contain more songs than others. Since the
lexicon is skewed towards negative sentiment, lengthier albums have more
chances of having negative words, which drags the total score down.
Therefore, we will also perform a relative analysis using the average
instead of the sum of scores:
...
average score, with the two previously mentioned albums still
maintaining a positive score. However, since these are average scores,
some changes are noticeable. 808s & Heartbreak mantains the highest
positive average score, and Yeezus becomes the album with the lowest
average score, although followed somewhat closely by My Beautiful Dark
Twisted Fantasy.
...
is a co-appearance of both positive and negative feelings, reflecting
the emotional diversity in Kanye's music. Emotions such as joy, anger,
and trust appear regularly, suggesting a broad spectrum of emotional
exploration.

The analysis also highlights a consistent presence of negative
sentiment, with the two exceptions of 808s & Heartbreak (2008) and Kids
See Ghosts (2018). The evolution of sentiment over time demonstrates the
emotional shifts in Kanye’s music, with the latest albums leaning more
heavily on negative tones.
...
which probably contributes to the negative tone of his lyrics. This will
...
As can be seen, the albums Yeezus (2013) and My Beautiful Dark Twisted
Fantasy (2010), which had the lowest sentiment scores in the previous
AFINN analysis, also stand out in terms of the highest frequency of
insults. Similarly, the albums with positive sentiment scores appear to
be the ones with the least number of insults.
...
with a higher number of swear words generally have lower, more negative
sentiment scores.
...
"fuck", "ass", "bitch", "pussy", "motherfucker" and "dick", which we
believe is a common trend in American hip-hop by male artists.
...
appear several times across all of Kanye's albums.
...
words_by_line2 <- lines_df %>% # we retrieve this data frame that wasn't changed by tokenization or stopword removal
  unnest_tokens(word, line) %>%
  anti_join(custom_stop_words)

# Compute co-occurrence counts of words within lines
word_pairs2 <- words_by_line2 %>%
...
In Kanye West's work, his relationship with himself or how he perceives
himself in front of others is something easily recognizable. Through
songs like “I Am A God” we see a declaration of intentions in this
regard. We observe that the term “I” plays a major role in the lyrical
elaboration of the artist, finding numerous terms related to the
nighlife (fuck, money, gold, etc.). On the other hand, there is also a
strong relationship with God, which can be seen in the union of terms
such as “pray” or “god”. It is also interesting to note the relational
triangle that we find between “I”, “Me” and “My”, as there is a high
relationship between these terms. Those terms are positioned as the most
central vertices, indicating that Kanye focuses much of his lyric on the
exploration of the self. Around him, thematic clusters emerge where
words such as “feel”, “know”, “want” and ‘love’ appear, pointing to the
different nuances of his introspection.
...
accomplishments through nightlife reinforces the message of his songs.
...
"pray", etc. and constructed a graph that allows us to identify how 
these concepts dialogue with each other and with other topics (faith, 
redemption, guilt). This analysis facilitates understanding the density 
and context in which Kanye incorporates religious references in his lyrics.

```{r}
# Define a list of religion-related keywords
religious_words <- c("god", "jesus", "lord", "faith", "pray", "prayer", "prayers", "church", "gospel", "heaven", "blessings", "soul", "bless", "hallelujah", "blessed", "religious", "religion", "believer")
...
centers of gravity in the graph. In general, we observe an ambivalent
tone in Kanye's lyrics relating to religious words. While words linked
to "Jesus" make more contextual sense ("love", "save"), the term "God"
seems to be used in a broader sense (the stronger link with it forms
the expression "god damn", which probably carries no real religious
meaning).
In the same way, we do observe some unions of terms that make us
understand that the use of religiosity can sometimes border on the
profane in his lyrics. For example, the word "nightlife" appears in
the "God" cluster; the word "religion" is linked to the terms "drugs"
and "pussy" in Kanye's lyrics. Another one of the chosen words, "soul",
is probably not really used in the songs analyzed here in a religious 
sense.
The two words that are the core to the most important clusters, "God"
and "Jesus", appear together with "Devil's" and "Satan" respectively,
which reveals Kanye's tension or duality when discussing religion
or faith. 
However, we know that the topic of religion and the relationship to God
is further explored in his later work.
...
money_words <- c("money", "cash", "rich", "wealth", "dollars", "gold", 
                 "rolls", "bills", "chains", "luxury", "spend", "buy", "pay",
                 "cost")
...
The node “money” clearly stands out as the most connected, serving as
a direct bridge to multiple concepts associated with wealth. Around it
gravitate terms such as “pay”, ‘buy’ and “gold”, which form a primary
core of material exaltation. The words that appear in these clusters
are indeed the most materialistic of the graph, referring to very
particular, practical uses of money (weed, smoke, spaceship, insured,
lipo). Other words are joined with “money” to create expressions of 
abundance (pile, stack).

On the other hand, we see relationships at the money level that are
circumscribed to specific songs, as for example with the song “Gold
Digger” in which he talks about how a woman tries to get money from her
lovers for aesthetic operations, luxuries, etc. This explains certain
relationships that we see in the center of the “money” node.

Interconnected with this main group, thematic clusters appear that
combine deeper or conflicting reflections on the subject of money:
for example, nodes such as “power” are linked to “wealth” and ‘nobility’,
suggesting reflections on how money is power for the artist. The cluster
around "rich" is hard to decode: words like "cotton", "slave" or "fabric" 
emerge, perhaps alluding to slave labor or pointing to how people (or 
himself) are enslaved by money.

Overall, this network shows that the theme of money in Kanye West is
not merely decorative: it works in a direct way to narrate everything
that money can give you both on a more mundane level and from the
perspective of power. However, Kanye is not always the subject of
these expressions, but he also discusses how other subjects access
to and use their money.

# Final Considerations

Throughout this project, we used text mining techniques to explore Kanye
West's lyrics beyond just counting words, diving into themes like
identity, spirituality, and ambition. We analyzed almost a hundred of
his songs to spot frequently used words and examine how they connect,
uncovering deeper stories about who he is, what he believes in, and what
drives him. We aimed to unveil the changes, as well as the continuities,
in his lyrics over more than 15 years of production. We revealed a trend 
towards polarization in the sentiment expressed in Kanye's songs, with 
the distance between negative and positive words widening over time. We
also explored the use of slang, swear words and insults across his 
discography, and how these relate to the overall tone of his albums. 
Finally, we built some visualizations to convey the specific vocabulary 
and the type of language used by the artist in different key themes in his
music.

Despite these achievements, our study encountered several methodological
challenges worth noting. Firstly, the corpus was limited to Kanye's solo
albums, excluding collaborations, promotional singles, and recent works
after 2018. Including these additional texts in future research would
enhance the temporal breadth and stylistic understanding of Kanye’s 
evolving artistic direction. Furthermore, employing techniques such as
topic modeling (LDA, NMF) or phrase embeddings could further illuminate 
latent thematic structures not immediately evident through traditional
frequency analysis.

Secondly, inconsistencies in lemmatization during the text processing
phase sometimes resulted in artificially fragmented terms (e.g., "run"
vs. "running" vs. "ran"). Applying consistent lemmatization or stemming
prior to frequency and TF-IDF analysis, as well as word co-occurence,
would likely improve our lexical precision, ensuring that morphological 
variations are grouped appropriately and thus reflecting a more accurate 
portrayal of Kanye’s lyrical richness.

Thirdly, our sentiment analysis relied on lexicons such as Bing, NRC,
and AFINN, which have inherent limitations when analyzing lyrics rich 
in irony, double meanings, and specific hip-hop slang. To address 
these nuances more effectively, future studies should integrate other
advanced contextual language models like BERT or RoBERTa, capable of
capturing deeper semantic and emotional subtleties. Moreover, 
complementing our quantitative insights with qualitative analysis (such
as reviews, interviews, audience reception, and visual content) would
ground our findings in a richer socio-cultural context. This scope
should ensure that our conclusions go beyond statistical descriptions,
allowing a deeper understanding of how Kanye’s lyrics resonate
emotionally and culturally with diverse audiences over time.

In conclusion, this project demonstrates how text mining can illuminate
critical dimensions of artistic expression. The techniques employed 
provided us with greater knowledge and better understanding of Kanye
West's work and career. Nevertheless, as we have tried to make clear,
what has been done here is only a fraction of all the possibilities
offered by quantitative and qualitative methodologies,  and does not 
pretend to give a complete picture, but rather an exploration, of the
artist's personal and musical evolution.
